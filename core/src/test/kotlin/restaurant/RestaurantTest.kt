        ...
        testsAbout(Restaurant::class) {
                        ...
                        Restaurant {
                            namespace("/handlers") {
                                route(Method.POST, "reverser") { ex, _ ->
                                    response(ByteBuffer.wrap(ex.withBody().body!!.reversedArray()))
                                }
                            }
                        })
                it("exposes routes") {
            ...
            describe("error handling") {
                ...
                it("returns status 500 per default on error") {
                    val restaurant =
                        autoClose(Restaurant { route(Method.GET, "/", ExceptionsHandler()) })
            ...
            describe("async roundtrip") {
                it("works") {
                    ...
                    // the flow is still sending, but  we already got our answer. we must be
                    // streaming!
