        ...
        testsAbout(Restaurant::class) {
            describe("rest services") {
                it("empty responses return 204") {
                    ...
                    val restaurant =
                        autoClose(restaurant { resources(EmptyReplyService(), "/handlers/empty") })
                ...
                describe("rest routes") {
                    val restaurant =
                        autoClose(restaurant { namespace("/api") { resources(UserService()) } })

                    describe("post requests") {
                        val response =
                            restaurant.sendRequest("/api/users") { post("""{"name":"userName"}""") }
                    ...
                    it("calls update method on put request") {
                        val response =
                            restaurant.sendRequest("/api/users/5") {
                                put("""{"name":"userName"}""")
                            }
                        expectThat(response) {
                            get { statusCode() }.isEqualTo(200)
                    ...
                    it("calls delete method on delete request") {
                        val response = restaurant.sendRequest("/api/users/5") { delete() }
                        expectThat(response) {
                            get { statusCode() }.isEqualTo(200)
                    ...
                    describe("error handling") {
                        describe("malformed requests") {
                            it("returns a useful error message") {
                                ...
                                val response =
                                    restaurant.sendRequest("/api/users") { post(requestBody) }
                                expectThat(response) {
                ...
                describe("planned features") {
                    it(
                        "does not yet support nested routes",
                        ignored = Ignored.Because("not yet implemented")) {
                            val restaurant =
                                autoClose(
                                    restaurant {
                                        namespace("/api") {
                    ...
                    it(
                        "does not yet support singular routes",
                        ignored = Ignored.Because("not yet implemented")) {
